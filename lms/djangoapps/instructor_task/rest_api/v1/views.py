...
class ModifyScheduledBulkEmailInstructorTask(generics.DestroyAPIView, generics.UpdateAPIView):
    ...
    def patch(self, request, *args, **kwargs):
        ...
        try:
            task_schedule = self._get_task_schedule_with_id(schedule_id)
            task = task_schedule[0].task
            self._verify_task_state(task)
            ...
            with transaction.atomic():
                if schedule:
                    ...
                    task_schedule.update(task_due=schedule_utc)
                if email_data:
        ...
        except InstructorTaskSchedule.DoesNotExist as task_update_err:
            return Response(
                {"detail": str(task_update_err)},
    ...
    def _get_task_schedule_with_id(self, schedule_id):
        """
        Verifies that there is an instructor task schedule instance that exists with the specified id.
        """
        task_schedule = InstructorTaskSchedule.objects.filter(pk=schedule_id)
        if not task_schedule.exists():
            raise InstructorTaskSchedule.DoesNotExist(
